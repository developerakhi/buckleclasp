...
<div class="card">
    <form class="" action="" id="sort_orders" method="GET">
        <div class="card-header row gutters-5">
            <div class="col">
                <h5 class="mb-md-0 h6">All Customization</h5>
            </div>
        ...
        <div class="card-body">
                ...
                <tbody>
                   @foreach ($customizations as $key => $customization)
                    <tr>
                        ...
                        <td>{{ ($key+1) + ($customizations->currentPage() - 1)*$customizations->perPage() }}</td>
                        <td>
                            @php
                                $images= json_decode($customization->image,true);
                            @endphp
                        ...
                        <td>{{ $customization->customer_name }}</td>
                        <td>{{ $customization->customer_mobile }}</td>
                        <td>{{ $customization->customer_email }}</td>
                        <td>{{ $customization->description }}</td>

                        <td class="text-right">
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{ route('all_customization.show', $customization->id)}}"  title="show">
                            ...
                            <!--</a>-->
                            <a href="{{route('all_customization.destroy', $customization->id)}}" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('all_customization.destroy', $customization->id)}}" title="">
            ...
            <div class="aiz-pagination">
                {{ $customizations->appends(request()->input())->links() }}
            </div>
