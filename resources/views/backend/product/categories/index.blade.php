        ...
        <table class="table aiz-table mb-0">
            <thead>
                <tr>
                    ...
                    <th data-breakpoints="lg">{{ translate('New Status Level') }}</th>
                    <th data-breakpoints="lg">{{ translate('Level') }}</th>
                    <th data-breakpoints="lg">{{translate('Banner')}}</th>
                    <th data-breakpoints="lg">{{translate('Icon')}}</th>
                    <th data-breakpoints="lg">{{translate('Featured')}}</th>
                    <th data-breakpoints="lg">{{translate('New Status')}}</th>
            ...
            <tbody>
                @foreach($categories as $key => $category)
                    <tr>
                        ...
                        <td>{{ $category->new_status_level }}</td>
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input type="checkbox" onchange="update_new_status(this)" value="{{ $category->id }}" <?php if($category->new_status == 1) echo "checked";?>>
                                <span></span>
                            </label>
                        </td>
...
@section('script')
    <script type="text/javascript">
        ...
        function update_new_status(el){
            ...
            $.post('{{ route('categories.new_status') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Featured categories updated successfully') }}');
                }
                else{
